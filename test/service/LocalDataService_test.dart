...
void main() {
  ...
  group('loadAnimeDetails', () {
    ...
    test('Verifica se o método vai carregar corretamente os dados', () async {
      when(http.get(any)).thenAnswer((_) async => Response(animeDetalhesJson, 200));
      Anime anime = Anime();
      anime.id = 1064;
      anime = await api.loadAnimeDetails(anime, http);
      expect(anime.id, 1064);
      expect(anime.animeDetails.urlImagemGrande, isNotEmpty);
      expect(anime.animeDetails.genres.length, 3);
      expect(anime.animeDetails.studios.length, 1);
      expect(anime.animeDetails.studios[0], "Brain's Base");
      expect(anime.animeDetails.openings[0], "Fire Wars by Jam Project");
      expect(anime.animeDetails.endings[0], "Tornado by JAM Project featuring Ichiro Mizuki");
    });
  });
